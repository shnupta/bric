...
sigwinch_handler ()
{
  ...
  if (get_window_size
      (STDIN_FILENO, STDOUT_FILENO, &Editor.screen_rows,
  ...
  else
  {
    ...
    {
      int change =
        (delta > Editor.column_offset) ? Editor.column_offset : delta;
...
int
editing_row_has_open_comment (editing_row * row)
{
  if (row->hl && row->rendered_size
      && row->hl[row->rendered_size - 1] == HL_MLCOMMENT
      && (row->rendered_size < 2
          || (row->rendered_chars[row->rendered_size - 2] != '*'
              || row->rendered_chars[row->rendered_size - 1] != '/')))
    return 1;
...
editor_update_syntax (editing_row * row)
{
  ...
  if (row->index > 0 && editing_row_has_open_comment (row->prev))
    in_comment = 1;
    ...
    if (prev_sep)
    {
      ...
      for (j = 0; keywords[j]; j++)
      {
        ...
        if (!memcmp (p, keywords[j], klen) && is_separator (*(p + klen)))
        {
          // keyword
          if (pp)
            memset (row->hl + i, HL_KEYWORD_PP, klen);
          else if (cond)
            memset (row->hl + i, HL_KEYWORD_COND, klen);
          else if (retu)
            memset (row->hl + i, HL_KEYWORD_RETURN, klen);
          else if (adapter)
            memset (row->hl + i, HL_KEYWORD_ADAPTER, klen);
          else if (loopy)
            memset (row->hl + i, HL_KEYWORD_LOOP, klen);
          else
            memset (row->hl + i, HL_KEYWORD_TYPE, klen);
          p += klen;
          i += klen;
          break;
        }
  ...
  if (row->hl_open_comment != oc && row->index + 1 < Editor.num_of_rows)
    editor_update_syntax (row->next);
  row->hl_open_comment = oc;
}

// map thee syntax highlight types to terminal colours
int
editor_syntax_to_colour (int highlight)
{
  switch (highlight)
  {
  case HL_COMMENT:
    return Editor.colours.hl_comment_colour;
  case HL_MLCOMMENT:
    return Editor.colours.hl_mlcomment_colour;
  case HL_KEYWORD_COND:
    return Editor.colours.hl_keyword_cond_colour;
  case HL_KEYWORD_TYPE:
    return Editor.colours.hl_keyword_type_colour;
  case HL_KEYWORD_PP:
    return Editor.colours.hl_keyword_pp_colour;
  case HL_KEYWORD_RETURN:
    return Editor.colours.hl_keyword_return_colour;
  case HL_KEYWORD_ADAPTER:
    return Editor.colours.hl_keyword_adapter_colour;
  case HL_KEYWORD_LOOP:
    return Editor.colours.hl_keyword_loop_colour;
  case HL_STRING:
    return Editor.colours.hl_string_colour;
  case HL_NUMBER:
    return Editor.colours.hl_number_colour;
  case HL_MATCH:
    return Editor.colours.hl_match_colour;
  case HL_BACKGROUND_DEFAULT:
    return Editor.colours.hl_background_colour;
  default:
    return Editor.colours.hl_default_colour;
  }
}
...
void
editor_update_row (editing_row * row)
{
...
// insert row
void
editor_insert_row (int at, char *s, size_t length)
{
...
void
editor_free_row (editing_row * row)
{
  free (row->rendered_chars);
  free (row->chars);
  free (row->hl);
...
void
editor_row_insert_char (editing_row * row, int at, int c)
{
  if (at > row->size)
  {
...
void
editor_row_append_string (editing_row * row, char *s, size_t len)
{
...
void
editor_row_delete_char (editing_row * row, int at)
{
  if (row->size <= at)
    return;
...
editor_insert_char (int c)
{
  ...
  editing_row *row =
    (filerow >= Editor.num_of_rows) ? NULL : find_row (filerow);
...
char *
get_indent_prefix (char *s, int max_length)
{
  int ptr = 0;
  while (s[ptr] == ' ' || s[ptr] == '\t')
    ptr++;
...
char *
set_indent_prefix (char *text, char *prefix)
{
...
editor_copy_row ()
{
  ...
  if (row)
  {
    ...
    {
      if ((Editor.yank_buffer =
           (char *) malloc (Editor.yank_buffer_len)) != NULL)
      {
    ...
    {
      if ((Editor.yank_buffer =
           (char *) realloc (Editor.yank_buffer,
                             Editor.yank_buffer_len)) != NULL)
      {
        memcpy (Editor.yank_buffer, row->chars, Editor.yank_buffer_len);
      }
      else
...
writeerr:
  ...
  if (fd != -1)
    close (fd);
  editor_set_status_message (_("Cannot save! I/O error: %s"),
                             strerror (errno));
  return 1;
}
...
void
ab_append (struct append_buf *ab, char *s, int length)
{
...
editor_refresh_screen (void)
{
  ...
  for (y = 0; y < Editor.screen_rows; y++)
  {
    ...
    if (filerow >= Editor.num_of_rows)
    {
      if (Editor.num_of_rows == 1 && y == Editor.screen_rows / 3
          && Editor.mode != INSERT_MODE && !Editor.dirty && Editor.newfile)
      {
        char welcome[80];
        /* TRANSLATORS:  "Bric" is the program name and should not be translated */
        int welcomelen = snprintf (welcome, sizeof (welcome),
                                   _("Bric editor -- version %s\x1b[0K\r\n"),
                                   VERSION);
    ...
    if (len > 0)
    {
      ...
      for (j = 0; j < len; j++)
      {
        if (Editor.mode == SELECTION_MODE
            && is_char_selected (j + Editor.column_offset, filerow))
        {
  ...
  int len = snprintf (status, sizeof (status),
                      ...
                      ngettext ("%.50s =- %d line %s",
                                "%.50s =- %d lines %s",
                                Editor.num_of_rows),
                      /* TRANSLATORS: modified = changed file */
                      Editor.filename, Editor.num_of_rows,
                      Editor.dirty ? _("(modified)") : "");
  int rlen = snprintf (rstatus, sizeof (rstatus), "%d/%d",
                       Editor.row_offset + Editor.cursor_y + 1,
                       Editor.num_of_rows);
  if (len > Editor.screen_columns)
    len = Editor.screen_columns;
  ...
  if (message_length && time (NULL) - Editor.status_message_time < 5)
    ab_append (&ab, Editor.status_message,
               message_length <=
               Editor.
               screen_columns ? message_length : Editor.screen_columns);

  // put cursor at its current position
  int j;
  int cursor_x = 1;
  if (Editor.line_numbers)
    cursor_x += line_number_length;
  int filerow = Editor.row_offset + Editor.cursor_y;
  editing_row *r =
    (filerow >= Editor.num_of_rows) ? NULL : find_row (filerow);
  if (r)
  {
    int was_tab = 0;
    for (j = Editor.column_offset;
         j < (Editor.cursor_x + Editor.column_offset); j++)
    {
...
#define FIND_RESTORE_HL do { \
  ...
  int saved_column_offset = Editor.column_offset, saved_row_offset =
    Editor.row_offset;
...
#define FIND_RESTORE_HL do { \
  ...
  while (1)
  {
    editor_set_status_message (_
                               ("Search: %s Replace: %s (Use ESC/Tab/Arrows/Enter)"),
                               query, replace_word);
    editor_refresh_screen ();
...
editor_check_quit (int fd)
{
  ...
  {
    editor_set_status_message (_
                               ("There are unsaved changes, quit? (y or n) %s"),
                               query);
    editor_refresh_screen ();
...
handle_tag_movement (int where)
{
  ...
  if (where == MOVE_BACK)
  {
    ...
    for (i = 0;
         i < cursor_pos
         && Editor.row_offset + Editor.cursor_y < Editor.num_of_rows - 1; i++)
      editor_move_cursor (ARROW_DOWN);
    for (; i > 0; i--)
      editor_move_cursor (ARROW_UP);
    for (i = 0;
         i < cursor_pos && Editor.row_offset + Editor.cursor_y > 0; i++)
      editor_move_cursor (ARROW_UP);
    for (; i > 0; i--)
      editor_move_cursor (ARROW_DOWN);
    return 1;
...
void
editor_parse_command (int fd, char *query)
{
...
void
editor_process_key_press (int fd)
{
  ...
  case NORMAL_MODE:
    ...
    {
    case 'r':
      if (Editor.prev_char == 'c')
        editor_copy_row ();
      if (Editor.prev_char == 'y')
        editor_yank_row ();
      if (Editor.prev_char == 'p')
        editor_paste_row ();
      if (Editor.prev_char == 'd')
        editor_delete_row (filerow);
      break;
    case 'p':
      if (Editor.prev_char == 'c')
...
main (int argc, char **argv)
{
  ...
  {
    fprintf (stderr,
             _("The file has been locked, try to remove the locker!\n"));
    return EXIT_FAILURE;
  }
